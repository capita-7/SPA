...
echo -e "\n ${BOLD}${GREEN}######################################### ${RESET}\n spa-term is Installing the Environment for Kubernetes ☸️ ! \n  ${BOLD}${GREEN}######################################### ${RESET}"
...
echo -e "\n${BOLD}${BLUE} All Prerequisites are Satified ! all set to Run spa Playbook."${RESET}

### Installing kubeBench
echo -e "\n${BOLD}${BLUE} Initiating spa Playbook for KubeBench.."${RESET}

echo -e "\n${BLUE} Creating spaecure Namespace"${RESET}
kubectl create ns spaecure
sleep 10

if [ $? -eq 0 ]; then
    echo -e "\n${BLUE}Creating Kube bench job in spaecure Namespace"${RESET}
    kubectl apply -f https://raw.githubusercontent.com/samirparhi-dev/kube-bench/main/job.yaml -n spaecure
    sleep 30
    kubectl get jobs -n spaecure
    if [ $? -eq 0 ]; then
        ...
        kubectl logs $POD_NAME -n spaecure
    fi
else
    echo -e "\n ${BLUE}Could not able to Create NameSpace , Try Again"${RESET}
fi
echo -e "\n${BOLD}${BLUE}Thank you SoMuch for Usin spa tool.\n You can Share your thoughts and Improvements by creating PRs on out GitHub.https://github.com/capita7/spa"${RESET}